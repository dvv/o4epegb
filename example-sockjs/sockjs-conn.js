...
console.log('INMESSAGE', message);
  ...
  try {
    ...
    if (isArray(args = Connection.decode(message))) {
      ...
      this.socket.server.emit.apply(this.socket.server, ['wsevent', this].concat(args));
    // data?
    } else {
      // emit 'data' event
      this.emit('data', args);
      this.socket.server.emit('wsdata', this, args);
    }
  } catch(e) {
...
console.error('SOCKETERROR', error, this);
  this.socket.server.emit('wserror', this, error);
};
