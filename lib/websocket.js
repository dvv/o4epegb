...
console.log('SOCKET: close');
  delete this.conn;
  this.expireAt = Date.now() + Connection.STALE_TIMEOUT;
}
    ...
    // event?
      ...
      if (isArray(args = Connection.decode(message))) {
        ...
        // ordinary events
          ...
          conn.server.emit.apply(conn.server, [conn].concat(args));
        }
      // data?
      } else {
        // emit 'data' event
        conn.emit('data', args);
        conn.server.emit('data', conn, args);
      }
    }
  } catch(e) {
...
setInterval(function() {
  for (var i = 0; i < conns.length; ++i) {
    if (conns[i].socket.destroyed &&
      conns[i].socket.expireAt < Date.now()) {
      conns[i].remove();
    }
  }
console.error('GC:', conns.length, require('os').freemem());
}, 5000);
...
///
conns.push(this);
console.error('CONNECTION ADDED:', this.id, conns.length);
///
  this.socket.server.conns[this.id] = this;
  this.socket.server.emit('wsconnection', this);
  this.emit('connect');
  return this;
};
...
console.error('CONNECTION REMOVED:', this.id, conns.length);
///
  delete this.socket.server.conns[this.id];
...
/**
 * Timeout in ms for removing stale connection -- which have
 * the destroyed transport and get no new transport.
 *
 * @api private
 */

Connection.STALE_TIMEOUT = 120000;
