...
function handleSocketMessage(message) {
  ...
  var conn = server.conns[this.id];
  var args;
console.log('SOCKET: message', message, this.id, !!conn);
  try {
    // authorization?
    if (!this.id) {
      var id = message;
      ...
      // connection exists?
      if (conn) {
console.log('REPLACING SOCKET FOR', this.id);
        // replace underlying socket
        conn._s.end();
        conn._s = this;
        conn.authorize(id, function(err, id) {
          if (err) {
            conn.close();
          } else {
            self.id = id;
...
console.log('REGISTERING NEW SOCKET FOR', this.id);
    ...
    // event?
      ...
      if (args[0] === Connection.SERVICE_CHANNEL + 'disconnect') {
        delete server.conns[this.id];
        this.end();
        server.emit('wsclose', this.conn);
        conn.emit('close');
      // other events
      } else {
        server.emit('wsmessage', this.conn, args);
        conn.emit('message', args);
      }
    // data?
   } else {
      // emit 'data' event
      server.emit('wsdata', this.conn);
      conn.emit('data', args);
    }
  } catch(e) {
...
Connection.prototype.send = function(/* args... */) {
  ...
  // TODO: rpush('queue:' + this._s.id, args);
...
Connection.prototype.flush = function() {
  ...
  // TODO: blpop('queue:' + this._s.id, args);
