...
//console.log('MESSAGE', event.data);
  try {
    ...
    if (isArray(args)) {
      ...
      if (aid &&
          aid.substring(0, Connection.SERVICE_CHANNEL.length)
...
Connection.prototype.open = function(url) {
  ...
  if (this.ws && this.ws.readyState === 0) return this;
  ...
  this.ws.conn = this;
  this.ws.onopen = bind(handleSocketOpen, this.ws);
  this.ws.onclose = bind(handleSocketClose, this.ws);
  this.ws.onerror = bind(handleSocketError, this.ws);
  this.ws.onmessage = bind(handleSocketMessage, this.ws);
  return this;
};
