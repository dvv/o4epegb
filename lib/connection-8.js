...
Connection.gc = function(conn) {
  setTimeout(function() {
    conn.expireStaleOneTimeListeners(now());
    if (conn.live) Connection.gc(conn);
...
Connection.prototype.once = function(name, fn, expireAt) {
  var self = this;
  function on() {
  ...
  on.listener = fn;
  if (expireAt) on.expireAt = expireAt;
...
/**
 * Expiry the listener if it had been given expireAt member and
 * this member is less than specified parameter.
 *
 * @param olderThan Number Timestamp. If falsy, expire unconditionally
 *
 * @api private
 */
...
Connection.prototype.expireStaleOneTimeListeners = function(olderThan) {
  if (this._events) for (var n in this._events) {
    ...
    for (var i = 0, l = list.length; i < l; i++) {
      if (list[i].listener &&
        (!olderThan || list[i].expireAt < olderThan)) {
console.log('EXPIRE', n);
...
console.log('INMESSAGE', message);
  try {
    ...
    if (!this.live) {
      // the very first message contains connection id
      this.id = message;
...
function handleSocketClose() {
  ...
  // reconnect is disabled!
    ...
    delete this.live;
    this.emit('close');
    // expire pending acks, because closed connection which is
    // not marked to reconnect means communication is done and no
    // further data will arrive. Hence, all pending acks must expire
    this.expireStaleOneTimeListeners();
  }
};
...
Connection.prototype.open = function(url) {
  ...
  // TODO: typeof MozWebSocket !== 'undefined' ? MozWebSocket : WebSocket
